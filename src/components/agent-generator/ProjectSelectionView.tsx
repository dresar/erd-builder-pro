...
import { Users, Search, FolderKanban, ChevronRight, Database, Layers, FileText, Sparkles, FolderPlus } from 'lucide-react';
  ...
  return (
    <div className="flex-1 flex flex-col gap-4 p-4 sm:p-6 overflow-y-auto custom-scrollbar max-w-6xl mx-auto w-full">
      ...
      {filteredProjects.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 sm:p-12 text-center rounded-2xl border border-dashed border-border/70 bg-card/50 space-y-4 my-auto">
          ...
          <div className="space-y-1 max-w-sm">
            <h3 className="text-sm font-semibold text-foreground">
              {searchProjectQuery ? 'Proyek Tidak Ditemukan' : 'Belum Ada Proyek'}
            </h3>
            <p className="text-xs text-muted-foreground leading-relaxed">
              {searchProjectQuery
                ? 'Tidak ada proyek yang sesuai dengan kata kunci pencarian.'
                : 'Buat proyek terlebih dahulu atau buat spesifikasi lengkap menggunakan AI Generator.'}
            </p>
          </div>
          <div className="flex items-center gap-2 pt-2">
            <Button
              ...
              className="h-8 px-3 text-xs bg-indigo-600 hover:bg-indigo-700 text-white font-medium cursor-pointer"
            >
