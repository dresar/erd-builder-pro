...
Use only type "single" or "multiple", two to seven concise options, exactly one question, and a required recommendedOption that exactly matches one listed option. The recommendedOption is a reference to an option, not free text: copy the selected option character-for-character, including every word, case, space, and punctuation mark; never paraphrase, translate, shorten, or add words. Before sending, compare recommendedOption with options and regenerate it if there is no exact match. For type "multiple", still provide exactly one recommendedOption and never join options with semicolons or emit multiple recommendations. Always set allowCustom to true. When present, use the Planning context as the source of user-confirmed decisions; newer entries override older ones. Treat [Plan feedback] as authoritative: skip means defer this decision and continue with the next highest-impact area, not-relevant means discard the question premise, undecided means record it as deferred, recommend means provide one best-fit recommendation then ask for confirmation, correct-context means use the correction as verified context, and finish-with-assumptions means write the final plan now while clearly labeling unresolved material choices as assumptions. For a final PRD, write Markdown directly: never wrap the whole response in a markdown code fence, never nest one fenced code block inside another, and fence only the required DBML block as \`\`\`dbml with one matching closing fence. If you write the final plan, do not append a plan-question block and do not include a "Open questions" or "Pertanyaan terbuka" section for material decisions; ask those interactively first unless the user selected finish-with-assumptions. Structure the final PRD with: Goal and success outcome, validated decisions, MVP scope (in and out), user roles and main flows, business rules, technical architecture including stack, authentication, RBAC, security and data/integrations, delivery phases, risks/trade-offs, and explicitly labeled non-blocking assumptions or deferred decisions. Never invent a user decision: state recommendations and assumptions as such. Continue in the user's language.`;
  },
};
